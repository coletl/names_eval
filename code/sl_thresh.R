...
# Description: Search for threshold for homogenous sublocations to use for name estimation.
...
# saveRDS(vr12, "data/vr12.rds")
# no_rv12 <- vr12[ , .(no_rv12 = .N), by = psid12]
# saveRDS(no_rv12, "data/no_rv12.rds")

sl_tprop <- readRDS("data/sl_tribe_prop.rds")
...
sl3 <- left_join(sl2, sl_tprop, by = c(SLID = "sl_id"))

# Calculate number of registered voters in each homogenous subloc
linter <- st_intersects(sl3, ps2)
sl3$no_rv07 <- sapply(linter, function(x) sum(ps2[x, ]$no_rv07, na.rm = TRUE))
sl3$no_rv12 <- sapply(linter, function(x) sum(ps2[x, ]$no_rv12, na.rm = TRUE))

# Subset to homogenous sublocations
test <- filter(sl3, max_prop > 0.95) %>% 
  group_by(plur_eth) %>% summarize(no_rv07 = sum(no_rv07),
                                   no_rv12 = sum(no_rv12)) %>% 
  arrange(no_rv07)
